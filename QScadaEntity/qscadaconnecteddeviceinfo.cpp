    ...
    for (QScadaDeviceInfo *info: deviceList) {
        ...
        for(QScadaBoard *board : boardController->getBoardListForDeviceIp(info->ip().toString())) {
            ...
            for (QScadaObject *object : *board->objects()) {
                ...
                rDevices += i.formTag(tag_object, true, true, 3);
            }
            rDevices += i.formTag(tag_board, true, true, 2);
        }
        rDevices += i.formTag(tag_boards, true, true, 1);
        rDevices += i.formTag(tag_device, true, true, 1);  // ← device closes after boards
    }
